...
const DEFAULT_LEVEL_CONFIGS: TrustLevelConfig[] = [
  {
    ...
    description: 'Agent with excellent track record - minimal oversight required',
    threshold: {
      minExecutions: 50,
      successRate: 0.95,
      averageScore: 90,
  ...
  },
  {
    ...
    description: 'New agent or agent recovering from failures - enhanced supervision',
    threshold: {
      minExecutions: 0,
      successRate: 0,
      averageScore: 0,
...
export class TrustCascadeEngine {
  ...
  constructor(projectRoot: string, options: TrustEngineOptions = {}) {
    this.storePath = options.storePath || path.join(projectRoot, '.gemini', 'trust-scores.json');
    this.maxHistorySize = options.maxHistorySize || 100;
    this.autoSave = options.autoSave !== false;
    this.onLevelChange = options.onLevelChange;
    
    // Merge custom configs with defaults
    this.levelConfigs = this.mergeConfigs(DEFAULT_LEVEL_CONFIGS, options.levelConfigs);
  ...
  calculateTrustLevel(agentId: string): TrustLevel {
    ...
    for (const config of this.levelConfigs.filter(c => c.level !== TrustLevel.L0_QUARANTINE)) {
   ...
   */
  getSummary(): Array<{ agentId: string; level: TrustLevel; levelName: string; metrics: TrustMetrics }> {
   ...
   */
  recordExecution(agentId: string, result: {
    success: boolean;
    qualityScore: number;
    durationMs?: number;
    ...
    const totalScores = metrics.averageQualityScore * (metrics.totalExecutions - 1) + result.qualityScore;
    ...
    const recordResult: ExecutionRecordResult = {
      ...
      changeDirection: levelChanged 
        ? (newLevel > previousLevel ? 'promoted' : 'demoted')
        : undefined,
      metrics,
    };
  ...
  setTrustLevel(agentId: string, level: TrustLevel, reason: string): void {
    ...
    if (level === TrustLevel.L0_QUARANTINE) {
      metrics.criticalFailures.push(`[${new Date().toISOString()}] Manual quarantine: ${reason}`);
    }
    ...
    console.log(`[Trust] Agent "${agentId}" manually set to ${TrustLevel[level]}: ${reason}`);
  }
  ...
  private meetsThreshold(
    ...
    if (threshold.zeroCriticalFailures && metrics.criticalFailures.length > 0) return false;
  ...
  private mergeConfigs(
    ...
    custom?: Partial<TrustLevelConfig>[]
  ): TrustLevelConfig[] {
  ...
  private loadMetrics(): void {
    try {
      if (fs.existsSync(this.storePath)) {
        ...
        console.log(`[Trust] Loaded metrics for ${this.agentMetrics.size} agents`);
      }
    } catch (error) {
      console.warn('[Trust] Could not load trust metrics, starting fresh:', error);
