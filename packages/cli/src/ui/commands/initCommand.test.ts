...
describe('initCommand', () => {
  ...
  const geminiMdPath = path.join(targetDir, 'DEVORA.md');
  ...
  it('should inform the user if DEVORA.md already exists', async () => {
  ...
  it('should create DEVORA.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty DEVORA.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
    ...
    expect(result.content).toContain(
      'You are an AI agent that brings the power of Devora',
    );
  });
