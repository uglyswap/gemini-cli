...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (DEVORA.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory refresh loads DEVORA.md files. When
...
(defaulting to `DEVORA.md` but configurable via the `context.fileName` setting)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  ...
  1.  **Global context file:**
      - Location: `~/.gemini/<configured-context-filename>` (e.g.,
        `~/.gemini/DEVORA.md` in your user home directory).
